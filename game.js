...
const TARGET_FPS = 50; // cap at 50fps to reduce lag
const FRAME_MS   = 1000 / TARGET_FPS;

function loop(now) {
  if (!state) return;
  const elapsed = now - state.lastFrame;
  if (elapsed < FRAME_MS) { raf = requestAnimationFrame(loop); return; }
  const dt = Math.min(elapsed, 80) / 1000; // clamp dt so tabbing away doesn't cause huge jumps
  state.lastFrame = now - (elapsed % FRAME_MS);
  state.timeLeft -= dt;
