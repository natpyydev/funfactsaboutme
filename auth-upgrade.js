...
// ============================================================
// LAZY AUTH — only grab after Firebase app is ready
// ============================================================
let _auth = null;
function getAuthLazy() {
  if (!_auth) {
    try { _auth = getAuth(getApp()); } catch (e) {
      console.warn('[auth-upgrade] Firebase app not ready yet:', e.message);
    }
  }
  return _auth;
}
...
async function linkWithGoogle() {
  ...
  try {
    ...
    if (err.code === 'auth/credential-already-in-use') {
      setStatus('⚠️ that Google account is already used — signing you in & restoring progress…', '#e67e22');
      try {
        const result = await signInWithPopup(auth, provider);
        const newUID = result.user.uid;
        await mergeProgressIfNeeded(oldUID, newUID);
        setStatus('✅ signed in with Google! progress restored 💗', '#27ae60');
        onLinkedSuccess();
      } catch (e2) {
