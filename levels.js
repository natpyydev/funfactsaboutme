...
// ============================================================
// 🔄 REALTIME SYNC (Firebase → Local State → UI)
// ============================================================
function startRealtimeSync() {
  ...
  const uid = db.getUID();
  if (!uid) { setTimeout(startRealtimeSync, 500); return; }

  db.onValue(db.ref(db.db, 'userLevels/' + uid), (snap) => {
    const fb = snap.val();
    if (!fb) return;

    // 🚨 overwrite LOCAL state with Firebase
    state.level = fb.level || 1;
    state.exp = fb.exp || 0;
    state.plate = fb.plate || 'paper';

    // 🧠 unlock plates based on level
    state.unlocked = PLATES
      .filter(p => state.level >= p.unlock)
      .map(p => p.id);

    // 💾 save to localStorage so UI uses correct data
    try {
    ...
    // 🔔 notify plate change
    window.dispatchEvent(new CustomEvent('plate-changed', {
      detail: { plate: state.plate }
    }));
  });
}

// start it
setTimeout(startRealtimeSync, 1200);
